...
import SdobSelectTeamButton from './components/SdobSelectTeamButton';
import SdobSelectVideoButton from './components/SdobSelectVideoButton';
  ...
  return (
    <>
      ...
      >
        ...
        {(!sdob && simpleMode) && <div role="button" onClick={toggleSimpleMode} style={{ marginLeft: 5, fontSize: '90%' }}>{t('Toggle advanced view')}</div>}
        ...
        {!sdob && !simpleMode && <ToggleExportConfirm style={{ marginRight: 5 }} />}

        {!sdob && <ExportButton size={1.3} segmentsToExport={segmentsToExport} areWeCutting={areWeCutting} onClick={onExportPress} />}
      </div>
    </>
  );
});
