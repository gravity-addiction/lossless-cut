...
function useFfmpegOperations({ filePath, enableTransferTimestamps }) {
  ...
  const autoConcatCutSegments = useCallback(async ({ customOutDir, isCustomFormatSelected, outFormat, segmentPaths, ffmpegExperimental, onProgress, preserveMovData, movFastStart, autoDeleteMergedSegments, chapterNames, preserveMetadataOnMerge, appendFfmpegCommandLog, nameSuffix }) => {
    const ext = getOutFileExtension({ isCustomFormatSelected, outFormat, filePath });
    const outPath = getOutPath({ customOutDir, filePath, nameSuffix: nameSuffix ? `${nameSuffix}${ext}` : `cut-merged-${new Date().getTime()}${ext}` });
